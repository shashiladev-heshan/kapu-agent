    ...
    tools: [
      ...
      tool(
        ...
        "Create a standing wish that runs autonomously (signed-in users only; restate plan + get a yes; ask about allow_order).",
        {
          ...
          cadence_kind: z.enum(["once", "daily", "weekly", "monthly", "yearly", "hours"]),
          at: z.string().optional().describe("HH:mm SL time (first/only run time)"),
          times: z.array(z.string()).optional().describe("daily: extra HH:mm times beyond 'at' — ['18:00'] for 2×/day"),
          every_hours: z.number().optional().describe("cadence_kind 'hours': every N hours (1-24)"),
