// GET /api/deals — the full kapruka.com/online/promotions list (~30 tiles,
// server-rendered catalogueV2 markup with true strikethrough prices) for the
// hero's standalone Hot-deals section. The client chunk-reveals on scroll.
// One ~0.8s fetch per 15 min; geo note: the promo tiles only render for
// Sri-Lankan IPs, so this returns [] from foreign egress (section hides).
...
export async function GET(): Promise<Response> {
  if (g.__kapuDeals && Date.now() - g.__kapuDeals.at < TTL) {
    return Response.json({ products: g.__kapuDeals.products });
  }
  try {
    ...
    if (!res.ok) return Response.json({ products: [] });
